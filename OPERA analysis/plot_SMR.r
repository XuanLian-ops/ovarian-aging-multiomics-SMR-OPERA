...
#  example:
       # source("D:/keti/plot/plot/plot_SMR.r")
       # smrdata=ReadSMRData("D:/keti/plot/plot/eqtl/eqtl_hypo_2021_plot.ENSG00000095485.txt")
       # # target probe passed pSMR
       # SMRLocusPlot(data=smrdata,smr_thresh = 3.179448047818899e-6,heidi_thresh = 0.05,plotWindow = 1000)
       # # target probe did not pass pSMR
       # SMRLocusPlot(data=smrdata,smr_thresh = 3.179448047818899e-6,heidi_thresh = 0.05,plotWindow = 1000)
       # # use "smr_thresh_plot" to show ILMN_2404135
       # SMRLocusPlot(data=smrdata,smr_thresh = 3.179448047818899e-6,heidi_thresh = 0.05,plotWindow = 1000, smr_thresh_plot=1e-1)
       # # use "probeNEARBY" to show ILMN_1724700
       # SMRLocusPlot(data=smrdata,smr_thresh = 8.4e-6,heidi_thresh = 0.05,plotWindow = 1000, probeNEARBY=c("ENSG00000095485","ENSG00000095485_does_not_exist"))
       # # use library of the third part to arrange the probe names
       # a2 <- SMRLocusPlot(data=smrdata,smr_thresh = 3.179448047818899e-6,heidi_thresh = 0.05,plotWindow = 1000,anno_selfdef=FALSE) # default anno_selfdef=TRUE
       # SMRLocusPlot(data=smrdata, smr_thresh=3.179448047818899e-6, heidi_thresh=0.05, plotWindow=2000, max_anno_probe=16)
       # # ===============================================================================
...
#调整基因标签的位置;设置文本的大小;'cex' 是 R 中用于控制绘图元素大小的参数;设置坐标轴标签的大小;
#设置坐标轴刻度标签的大小;用于设置图表顶部某些元素的大小，比如标题或顶部的标签
...
SMRLocusPlot = function(data=SMRData, probeNEARBY=NULL,smr_thresh=NULL, smr_thresh_plot=NULL, heidi_thresh=NULL, plotWindow=NULL,pointsize=20,max_anno_probe=16,anno_selfdef=TRUE)
{
    ...
    cis_start <- ifelse(is.na(cis_start) & cis_start < 0, 0, cis_start)
    ...
    for( k in 1 : nprobePLOT ) {
        ...
        if(length(which(smrprobes_red==probePLOT[k]))==0) {
            col_eqtl = "#ffc17f"#######maroon  756BB4   CD3A39
        } else col_eqtl = "#ffc17f"###ffc17f
    ...
    segments(xmin, ybuf, xmax, ybuf, col="#ffc17f", lty=2, lwd=1);
    text(xmax, ybuf+dev_anno, labels=strbuf, adj=1, col="#ffc17f", cex=axis,font=3);
}
